...
## RetroAchievements常见问题解答

### 什么是硬核模式？

如果你想要一比一地复刻过去你玩复古游戏的方式，那么硬核模式正是你所需要的。这个功能限制了玩家使用即时保存功能（在非硬核模式下，为了练习/票证目的可以使用该功能）、作弊以及任何改变游戏运行速度的功能，如倒带、慢动作，以及通过使用模拟器的内置功能一次一帧地推进游戏。 请注意，在该模式下唯一允许的改变游戏运行速度的方式是快进。基本上，你会在没有模拟器的游戏增强功能的条件下游玩游戏。
